...
public class BookViewAdapter extends RecyclerView.Adapter {
    ...
    RecyclerView의 Adapter를 객체로 생성할 때 화면에 보여줄 데이터를 전달하자(주입하자)
    ...
    */
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        View view = layoutInflater.inflate(
                R.layout.book_item_view, parent,
                false);
    ...
    RecyclerView에 의해서 데이터들의 개수만큼 반복적으로 호출이 되어 화면에 데이터를 그리는 작업을 수행
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ...
        /*
        매개변수로 받은 holder를 우리가 선언한
        BookItemHolder로 형변환을 시키고 이후 코드를 진행한다
         */
...
//        bookHolder.item_title.setText(bookDTO.getTitle());
...
//        bookHolder.item_desc.setText(bookDTO.getDescription());
        ...
        HTML에서는 단순히 img src="" 코드를 사용하여 연결하면 자동으로 다운로드를 받으면서
        이미지를 보여준다

        하지만 안드로이드에서는 디바이스 자체가 성능이 낮기 때문에 직접 이미지를 다운로드 하면
        문제를 일으킬 수 있다

        안드로이드에서 이미지 링크를 실제 이미지로 보여주는데
        Picasso, Glide와 같은 3rd party 라이브러리를 사용하여 화면에 이미지를 구현한다
    ...
    우리가 작성할 코드는
    item.xml 에 설정된 view Component 개별 요소를 추출하여 각각 객체로 만드는 일

    book_view_item.xml 에는 TextView가 있다
