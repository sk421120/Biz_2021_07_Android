...
public interface NaverRetrofit {
    ...
    /*
    2. naver openAPI를 사용하기 위해서는 반드시 header에
    Client ID와 Client Secret 값을 전달해 주어야 한다
    header에 Client ID와 Client Secret를 전달하기 위하여
        ...
        다. @Header() annotation에 Header의 이름을 지정한다
            X-Naver-Client-Id=sldfjklwei 형식으로 요청한다

    3. method에 end point와 요청 method type을 지정한다
        요청 method type : @GET @POST @PUT @DELETE

    4. openAPI를 통해 데이터를 요청할 때 값, 변수 등을 보내고 싶을때
    queryString을 만드는데 여기에서는 @Query()를 사용하여 지정한다

    5. return type인 Call 클래스에 Data를 mapping할 DTO(VO) 클래스를
    Generic으로 설정해 준다
     */
